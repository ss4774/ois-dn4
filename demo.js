	...
	if (!ehrId || ehrId.trim().length == 0) {
		...
		$.ajax({
	    	...
	    	success: function (data) {
				...
				$("#patient-age").html("<span class='obvestilo label label-success fade-in'>" + (date().getFullYear() - party.dateOfBirth.getFullYear()) + "</span>");
		...
		$.ajax({
		    ...
		    success: function (res) {
		    	if (res.length > 0) {
			        ...
			        //results += "</table>";
			        //$("#rezultatMeritveVitalnihZnakov").append(results);
			        $("#patient-dob").html("<span class='obvestilo label label-success fade-in'>" + res[0].weight + " " + res[0].unit + "</span>");
		    	} else {
		    		//$("#preberiMeritveVitalnihZnakovSporocilo").html("<span class='obvestilo label label-warning fade-in'>Ni podatkov!</span>");
